// src/services/job-websocket.service.js
import websocketManager from '@/utils/websocket-manager'

class JobWebSocketService {
  constructor() {
    // 如果是单例模式，检查是否已存在实例
    if (JobWebSocketService.instance) {
      return JobWebSocketService.instance
    }
    this.endpointName = 'job'
    this.endpointPath = '/ws/job'
    ...
    // 保存为单例
    JobWebSocketService.instance = this
  ...
  static getInstance() {
    if (!JobWebSocketService.instance) {
      JobWebSocketService.instance = new JobWebSocketService()
    }
    return JobWebSocketService.instance
  }
  ...
  async connect() {
    try {
      ...
      if (this.ws && this.ws.isConnected) {
        console.log('Job WebSocket 已连接，跳过重复连接')
        return true
      }
      ...
      console.error('Job WebSocket连接失败:', error)
      return false
    }
  }

  subscribeToTask(taskId, callback) {
    return this.ws.subscribe(`/topic/job/${taskId}`, callback)
  }
  unsubscribeToTask(subId) {
    return this.ws.unsubscribe(subId)
  }
  subscribeToTaskByUser(taskId, callback) {
    return this.ws.subscribe(`/user/job/${taskId}`, callback)
  }
  // subscribeTask(taskData) {
  //   return this.ws.send('/app/job/subscribe', taskData)
  // }
  // unsubscribeTask(taskData) {
  //   return this.ws.send('/app/job/unsubscribe', taskData)
  // }
...
export default JobWebSocketService.getInstance()
